...
size_t FakeInteger::current_number_of_operations[details::OPERATIONS_MONITORED] {};

struct details::QuotientAndRemainder {
  FakeInteger quotient;
  FakeInteger remainder;
};
...
inline FakeInteger operator+(const FakeInteger& val_1, const FakeInteger& val_2) {
  FakeInteger bigint(val_1);
  return bigint += val_2;
}
...
inline bool operator==(const FakeInteger& val_1, const FakeInteger& val_2) {
  // Both zeros.
  if (!val_1 && !val_2) {
    return true;
  }
...
inline std::istream& operator>>(std::istream& stream, FakeInteger& bigint) {
  bigint = 0;
  ...
  for (ssize_t i = num_str.length() - 1; i >= end; --i) {
    if (cur_pow_10 == FakeInteger::BASE) {
...
inline std::ostream& operator<<(std::ostream& stream, const FakeInteger& bigint) {
...
inline DIGIT_TYPE FindMinimalDigitForDivision(
    const FakeInteger& dividend,
    const FakeInteger& divisor) {
  // O(n log BASE).

  DIGIT_TYPE lower = 0, upper = FakeInteger::BASE;
  DIGIT_TYPE middle = 0;
  FakeInteger cur_bigint = 0;

  while (upper - lower > 1) {
    middle = (upper + lower) / 2;
    cur_bigint = divisor * static_cast<FakeInteger>(middle);
...
inline FakeInteger::FakeInteger()
  : is_negative_(false)
  , digits_(0) {}


inline FakeInteger::FakeInteger(int integer)
...
inline FakeInteger::FakeInteger(FakeInteger&& bigint) noexcept
...
inline FakeInteger& FakeInteger::operator=(FakeInteger&& bigint) noexcept {
  if (this == &bigint) {
    return *this;
  }
...
inline FakeInteger& FakeInteger::operator+=(const FakeInteger& bigint) {
...
inline std::string FakeInteger::toString() const {
...
inline FakeInteger::operator bool() const {
...
inline void FakeInteger::Add(const FakeInteger& bigint, const bool is_bigint_negative) {
...
inline std::pair<FakeInteger, FakeInteger> FakeInteger::SplitNumber(
    const FakeInteger& bigint,
    size_t digit_ind) {
  // O(n).

  FakeInteger first  = 0;
  FakeInteger second = 0;
...
inline FakeInteger FakeInteger::KaratsubaMultiply(
    const FakeInteger& bigint_1,
    const FakeInteger& bigint_2) {
  ...
  const std::pair<FakeInteger, FakeInteger> parts_1 = SplitNumber(bigint_1, middle);
  const std::pair<FakeInteger, FakeInteger> parts_2 = SplitNumber(bigint_2, middle);
  ...
  FakeInteger first  = KaratsubaMultiply(parts_1.second, parts_2.second);
  FakeInteger third  = KaratsubaMultiply(parts_1.first, parts_2.first);
  FakeInteger second = KaratsubaMultiply(parts_1.first + parts_1.second,
...
inline details::QuotientAndRemainder Divide(
  ...
  FakeInteger cur_divisor       = divisor;
  FakeInteger divisor_abs_value = divisor;
  divisor_abs_value.is_negative_ = false;

  if (!dividend) {
    return result;
  } else if (!cur_divisor) {
    throw(std::invalid_argument("Division by zero in FakeInteger."));
  }
  ...
  while (result.remainder >= divisor_abs_value) {
    ...
    result.remainder -= cur_divisor * static_cast<FakeInteger>(cur_digit);
